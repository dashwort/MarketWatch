...
namespace ThreeFourteen.Finnhub.Client.Runner
{
    class Program
    {
        ...
        static async Task Run()
        {
            ...
            //var companies = await client.Stock.SearchCompanies(client.Symbols);

            var a = await client.Stock.GetCompany2("A");
            var search = await client.Stock.SearchCompanies(client.Symbols);
            var b = client.Stock.OnSearchComplete += SearchIsDone;
            var c = await client.Stock.SearchSymbols("tlsa", true); ;
        ...
        private static void SearchIsDone(object sender, EventArgs e)
        {
            var s = sender as SearchResult;

            Trace.WriteLine($"Count {s.Count}");
        }
